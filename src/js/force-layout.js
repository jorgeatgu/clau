...
function forceLayout() {
    const chart = d3.select('.chart-force-maxima');
    ...
    function updateChart(dataz) {
        ...
        const node = svg
            .selectAll('.circle-maxima').remove().exit()
            .data(dataz)
            .enter()
            .append('circle')
            .attr('class', 'circle-maxima')
            .attr('r', d => d.radius)
            ...
            .on('mouseover', function (d) {
                ...
                d3.selectAll('.circle-maxima').filter(function (d, i) {
            ...
            .on('mouseout', () => {
                d3.selectAll('.circle-maxima').transition().duration(800).ease(d3.easeLinear)
                    .style('opacity', 1);
                tooltip.transition()
                    .duration(200)
                    .style('opacity', 0);
        ...
        const legend = svg.selectAll('.legend-maxima').remove().exit()
            .data(unique, d => d)
            .enter()
            .append('g')
            .attr('class', '.legend-maxima')
            .attr('year', d => d);
        ...
        legend.on('mouseover', function (tipo) {
                ...
                d3.selectAll('.legend-maxima')
                    .transition().duration(300).ease(d3.easeLinear)
                    .style('opacity', 0.1);
                legendThis
                    .transition().duration(300).ease(d3.easeLinear)
                    .style('opacity', 1);
                d3.selectAll('.circle-maxima')
